...
function NavItem({isExpanded, isActive, organization, onExpand}: NavItemProps) {
    ...
    const routes = [
        {
            label: 'Boards',
            icon: <Layout className='h-4 w-4 mr-3' />,
        ...
        },
        {
            ...
            href: `/org/${organization.id}/billings`,
        },
    ]
    ...
    return (
        <AccordionItem value={organization.id} className=" border-none">
            ...
            <AccordionContent className='pt-1 text-neutral-700'>
                {routes.map((route) => (
                    <Button key={route.href} size='sm' onClick={() => onClick(route.href)} variant='ghost' className={cn(
